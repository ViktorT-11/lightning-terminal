...
func getBBoltAccounts(db kvdb.Backend) ([]*OffChainBalanceAccount, error) {
	...
	err := db.View(func(tx kvdb.RTx) error {
		...
		readFn := func(k, v []byte) error {
			...
			// Also skip the kvdb deprecation marker bucket. We still want
			// to allow rerunning the kvdb -> SQL migration after the SQL
			// database has been deleted or downgraded, even though normal
			// bbolt startup should reject the tombstoned kvdb files.
